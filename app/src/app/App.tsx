...
export default function App() {
  ...
  return (
    <div className="app-shell">
      ...
      <ConfirmDialog open={removeRepositoryId !== null} title="Remove repository from project?" detail="The repository will be closed and removed from this project. Audits with a completed snapshot keep running; a snapshot still being prepared must finish first. An active agent may keep editing workspace files after the repository disappears from the sidebar." confirmLabel="Remove repository and keep active work running" onClose={() => setRemoveRepositoryId(null)} onConfirm={removeRepository} />
      <ConfirmDialog open={closeRepositoryId !== null} title="Close repository?" detail="Audits with a completed snapshot keep running, and an active agent may keep editing the workspace. A snapshot still being prepared must finish before the repository can close. The live repository view will close until you reopen it." confirmLabel="Close repository" onClose={() => setCloseRepositoryId(null)} onConfirm={async () => { if (!closeRepositoryId) return; try { setOperationError(null); await closeRuntimeRepository(closeRepositoryId, true); setCloseRepositoryId(null); } catch (error) { setOperationError(normalizeError(error)); } }} />
